    ...
    <Dialog open={Boolean(id)} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn(
          ...
          "h-[min(860px,calc(100svh-32px))] max-h-[calc(100svh-32px)] gap-0 overflow-hidden p-0 [display:flex] flex-col lg:w-[min(1180px,calc(100vw-32px))]",
        )}
      >
    ...
    <div className="space-y-2">
      ...
      <div className="grid min-w-0 gap-2 lg:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.75fr)] xl:grid-cols-[minmax(0,1.55fr)_380px]">
        <main className="min-w-0 space-y-2">
          ...
          <div className="grid gap-2 md:grid-cols-2">
            <section className={cn(componentOverlay.flatSection, "p-2")}>
              <h3 className="mb-1.5 text-xs font-semibold">商品</h3>
              <InfoGrid
                rows={[
                  ["类别", item.category],
                  ...
                  [
                    "颜色/容量",
                    [item.color, item.storage_capacity].filter(Boolean).join(" / ") || "-",
                  ],
            ...
            <section className={cn(componentOverlay.flatSection, "p-2")}>
              <h3 className="mb-1.5 text-xs font-semibold">检测</h3>
              <InfoGrid
                rows={[
          ...
          <section className={cn(componentOverlay.flatSection, "p-2")}>
            <h3 className="mb-1.5 text-xs font-semibold">附件凭证</h3>
            <div className="grid gap-1.5 sm:grid-cols-2">
              ...
              {data.attachments.length === 0 ? (
                <p className="text-xs text-muted-foreground">暂无附件凭证。</p>
              ) : null}
            </div>
          </section>
        </main>
        ...
        <aside className="min-w-0 space-y-2">
          ...
          <section className={cn(componentOverlay.flatSection, "p-2")}>
            <h3 className="mb-1.5 text-xs font-semibold">时间线</h3>
            <div className="space-y-1.5">
              ...
              {data.events.length === 0 ? (
                <p className="text-xs text-muted-foreground">暂无时间线记录。</p>
              ) : null}
            </div>
          </section>
        </aside>
  ...
  return (
    <section className={cn(componentOverlay.flatSection, "p-2")}>
      <div className="mb-1.5 flex min-w-0 items-start justify-between gap-2">
        <div className="min-w-0">
          <h3 className="text-xs font-semibold">回收来源</h3>
      ...
      <div className="mt-1.5 grid gap-1.5 xl:grid-cols-[minmax(0,1fr)_160px]">
        <div className="rounded-lg bg-[var(--surface-panel-muted)] px-2 py-1.5">
          <div className="mb-0.5 text-[10px] font-semibold text-muted-foreground">维修计划</div>
          <p className="line-clamp-2 text-xs font-medium">{summary.repairIssueSummary}</p>
          {summary.repairRows.length ? (
            <div className="mt-1 grid gap-1">
              {summary.repairRows.slice(0, 4).map((row) => (
                <div
                  key={row.key}
                  className="grid min-w-0 grid-cols-[minmax(0,1fr)_38px] gap-1.5 rounded-md bg-card px-1.5 py-1"
                >
                  <div className="min-w-0">
                    <p className="truncate text-[11px] font-medium">{row.label}</p>
        ...
        <div className="rounded-lg bg-[var(--surface-panel-muted)] px-2 py-1.5">
          <div className="mb-0.5 text-[10px] font-semibold text-muted-foreground">成本组成</div>
      ...
      <div className="mt-1.5 grid gap-1.5 md:grid-cols-2">
        <div className="rounded-lg bg-[var(--surface-panel-muted)] px-2 py-1.5">
          <div className="mb-0.5 text-[10px] font-semibold text-muted-foreground">凭证状态</div>
        ...
        <div className="rounded-lg bg-[var(--surface-panel-muted)] px-2 py-1.5">
          <div className="mb-0.5 text-[10px] font-semibold text-muted-foreground">风险 / 扣减</div>
          {summary.deductions.length ? (
            ...
            <div className="space-y-0.5">
              {summary.riskNotes.slice(0, 3).map((note) => (
                <p key={note} className="truncate text-[11px] text-status-warn-foreground">
                  {note}
                </p>
              ))}
            </div>
          ) : missingProof.length ? (
            <p className="text-[11px] text-status-warn-foreground">
              还缺 {missingProof.map((row) => row.label).join("、")}
            </p>
          ) : (
            <p className="text-[11px] text-muted-foreground">暂无风险或扣减记录。</p>
          )}
        </div>
      </div>
    </section>
...
function InventoryTransactionsSection({
  ...
  return (
    <section className={cn(componentOverlay.flatSection, "p-2")}>
      <div className="mb-1.5 flex min-w-0 items-start justify-between gap-2">
        <div className="min-w-0">
          <h3 className="text-xs font-semibold">财务流水</h3>
          <p className="mt-0.5 truncate text-[10px] text-muted-foreground">
      ...
      {transactions.length ? (
        ...
        <p className="rounded-lg bg-[var(--surface-panel-muted)] px-2 py-1.5 text-[11px] leading-4 text-muted-foreground">
...
function InfoBox({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="min-w-0 rounded-lg border border-[var(--border-panel)] bg-[var(--surface-panel-muted)] px-2 py-1.5">
      <div className="truncate text-[10px] leading-3 text-muted-foreground">{label}</div>
      <div className="mt-0.5 truncate font-mono text-[13px] font-semibold leading-4 tabular-nums">
        {value}
      </div>
    </div>
  );
  ...
  return (
    <dl className="grid gap-1 text-[11px] leading-4">
      {rows.map(([label, value]) => (
        <div
          key={label}
          className="grid grid-cols-[70px_minmax(0,1fr)] gap-2 sm:grid-cols-[76px_minmax(0,1fr)]"
        >
          <dt className="truncate text-muted-foreground">{label}</dt>
          <dd
            className={cn(
              "min-w-0 break-words font-medium",
              label === "备注" && "line-clamp-4 text-[10px] font-normal leading-4",
            )}
          >
            {value}
          </dd>
