...
test("order detail IMEI quick save persists across refresh", async ({ page }) => {
  ...
  await scannerDialog.getByPlaceholder("无法识别时可手动输入").fill(persistedImeiValue);
  await scannerDialog.getByRole("button", { name: "填入手动编号" }).click();
  ...
  await expect(devicePanel.getByText(persistedImeiValue)).toBeVisible();
  ...
  await detail.screenshot({
    path: `${screenshotDir}/imei-order-detail-save-refresh-desktop.png`,
  });
});
