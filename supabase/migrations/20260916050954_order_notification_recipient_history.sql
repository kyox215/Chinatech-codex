-- Preserve the selected recipient in scoped business history only.
-- Audit metadata, idempotency receipts and broadcast metadata remain free of the phone/body.
set lock_timeout = '5s';
...
begin
  ...
  insert into public.order_events(id,store_id,order_id,event_type,payload,operator_name,created_at)
    values(v_event_id,p_store_id,p_order_id,'message_sent',
      v_payload || case when v_phone is not null then jsonb_build_object('recipient_phone',v_phone) else '{}'::jsonb end,
      v_actor_name,v_now);
