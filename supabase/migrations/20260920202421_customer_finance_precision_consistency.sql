-- Forward-only correction for the already deployed customer-list v3 read model.
-- Amounts with more than two decimal places are review items, never receivables.
-- No historical order amount is rewritten.
...
raw_order_facts as (
  select
    ro.customer_id,
    ro.created_at,
    coalesce(ro.quotation_amount, 0)::numeric as quotation_amount,
    coalesce(ro.deposit_amount, 0)::numeric as deposit_amount,
    coalesce(ro.balance_amount, 0)::numeric as balance_amount,
    not (
      ...
      or lower(coalesce(ro.record_state::text, 'active')) <> 'active'
      or ro.deleted_at is not null
    ) as is_valid,
    coalesce(ro.is_paid, false) as is_paid,
    lower(coalesce(ro.payment_status::text, '')) as payment_status,
    lower(coalesce(ro.approval_status::text, '')) as approval_status,
    lower(coalesce(ro.approval_flow_status::text, '')) as approval_flow_status,
  ...
  left join public.order_workflow_statuses as workflow_status
    on workflow_status.store_id = ro.store_id
   and workflow_status.code = ro.status::text
),
order_facts as (
  select
    raw_order_facts.*,
    ...
    raw_order_facts.is_valid
      and raw_order_facts.quotation_amount > 0
      and raw_order_facts.balance_amount > 0
      and not raw_order_facts.is_paid
      and raw_order_facts.deposit_amount >= 0
      and raw_order_facts.quotation_amount = round(raw_order_facts.quotation_amount, 2)
      and raw_order_facts.deposit_amount = round(raw_order_facts.deposit_amount, 2)
      and raw_order_facts.balance_amount = round(raw_order_facts.balance_amount, 2)
      and raw_order_facts.deposit_amount + raw_order_facts.balance_amount
        <= raw_order_facts.quotation_amount
      and raw_order_facts.approval_flow_status <> 'rejected'
      and raw_order_facts.approval_status <> 'rejected'
      and raw_order_facts.approval_flow_status <> 'waiting_customer'
      and raw_order_facts.approval_status <> 'pending'
      and (
        raw_order_facts.approval_flow_status in ('approved', 'not_required')
        or raw_order_facts.approval_status = 'approved'
      )
      and (
        raw_order_facts.payment_status = ''
        or raw_order_facts.payment_status = case
          when raw_order_facts.deposit_amount > 0
      ...
      or (
        ...
        and exists (
          ...
          where unpaid_order.customer_id = customer.id
            and unpaid_order.is_collectible
        )
      )
      or (
...
order_stats as (
  select
    ...
    coalesce(sum(order_fact.quotation_amount) filter (
      where order_fact.is_valid
        and order_fact.quotation_amount >= 0
        and order_fact.quotation_amount = round(order_fact.quotation_amount, 2)
    ), 0)::numeric
      as lifetime_quoted_amount,
    coalesce(sum(order_fact.balance_amount) filter (where order_fact.is_collectible), 0)::numeric
...
global_stats as (
  select
    ...
    (
      ...
      from order_facts as unpaid_order
      where unpaid_order.is_collectible
    ) as unpaid,
    (
...
comment on function public.repairdesk_customer_list_page_v3(
  ...
  integer,
  integer
) is 'Customer page v3: receivables exclude cancelled, voided, deleted, paid, refunded, paid-balance conflicts, and amounts beyond two-decimal precision.';
