-- Atomic, idempotent self-service store creation.
-- The RPC is service-role-only and commits the store, lifecycle, owner,
-- canonical defaults and success audit as one PostgreSQL transaction.

alter table public.stores add column if not exists store_code text;

create table if not exists public.store_creation_operations (
  actor_id uuid not null,
  request_id uuid not null,
  request_hash char(64) not null,
  created_store_id uuid not null,
  completed_at timestamptz not null default now(),
  primary key (actor_id, request_id),
  constraint store_creation_operations_request_hash_check
    check (request_hash ~ '^[0-9a-f]{64}$')
);

alter table public.store_creation_operations enable row level security;
revoke all on table public.store_creation_operations from public, anon, authenticated;
grant select, insert on table public.store_creation_operations to service_role;
...
begin
  if coalesce(current_setting('request.jwt.claim.role', true), '') <> 'service_role' then
    raise exception using errcode = '42501', message = 'STORE_CREATE_FORBIDDEN';
  end if;
