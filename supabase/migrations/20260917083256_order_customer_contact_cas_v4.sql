-- Customer profile edits through an order require the version seen when editing.
-- No columns, historical snapshots, receipts or generated documents are backfilled.
-- Deploy this additive function before the v4 application. Older v3 clients can
-- replay existing receipts, but new customer writes fail closed until upgraded.
-- Rollback: stop customer editing or roll forward. Never restore non-CAS writes.
set lock_timeout = '5s';
set statement_timeout = '60s';

create or replace function public.repairdesk_mutate_order_v4(
  ...
  p_expected_customer_updated_at timestamptz default null
) returns jsonb
...
begin
  ...
  if exists (select 1 from jsonb_object_keys(p_order_changes) k where k not in (
    ...
    'warranty_text', 'warranty_months', 'warranty_change_reason',
  ...
  -- The original receipt above remains replayable, even for pre-CAS clients.
  if p_customer_changes <> '{}'::jsonb and p_expected_customer_updated_at is null then
    return jsonb_build_object('ok', false, 'code', 'customer_version_required');
  end if;
  if p_customer_changes ? 'name' and jsonb_typeof(p_customer_changes -> 'name') is distinct from 'string' then
    return jsonb_build_object('ok', false, 'code', 'invalid_fields');
  end if;
  v_phone_changed := p_customer_changes ?| array['phone_raw', 'phone_e164', 'contact_phones'];
  if p_customer_changes ?| array['phone_raw', 'phone_e164'] then
    if jsonb_typeof(p_customer_changes -> 'phone_e164') is distinct from 'string'
       or jsonb_typeof(p_customer_changes -> 'phone_raw') is distinct from 'string'
       or (p_customer_changes ->> 'phone_e164') !~ '^[+0-9][0-9 ().-]*$'
       or (p_customer_changes ->> 'phone_raw') !~ '^[0-9]{7,15}$'
       or pg_catalog.regexp_replace(p_customer_changes ->> 'phone_e164', '[^0-9]', '', 'g')
          is distinct from p_customer_changes ->> 'phone_raw' then
      return jsonb_build_object('ok', false, 'code', 'invalid_customer_phone');
    end if;
  end if;
  if p_customer_changes ? 'contact_phones' then
    if jsonb_typeof(p_customer_changes -> 'contact_phones') is distinct from 'array' then
      return jsonb_build_object('ok', false, 'code', 'invalid_customer_phone');
    end if;
    if jsonb_array_length(p_customer_changes -> 'contact_phones') > 20 or exists (
      select 1 from jsonb_array_elements(p_customer_changes -> 'contact_phones') x
       where jsonb_typeof(x) <> 'string' or (x #>> '{}') !~ '^[+0-9][0-9 ().-]*$'
          or pg_catalog.regexp_replace(x #>> '{}', '[^0-9]', '', 'g') !~ '^[0-9]{7,15}$'
    ) then return jsonb_build_object('ok', false, 'code', 'invalid_customer_phone'); end if;
  end if;

  -- Match create-order phone locks. Backup-only edits also acquire the sorted
  -- incoming number locks before order -> customer row locks.
  if v_phone_changed then
    select coalesce(array_agg(phone_key order by phone_key), '{}'::text[]) into v_phone_keys
      from (
        select distinct pg_catalog.regexp_replace(value, '[^0-9]', '', 'g') as phone_key
          from jsonb_array_elements_text(
            coalesce(p_customer_changes -> 'contact_phones', '[]'::jsonb)
            || case when p_customer_changes ? 'phone_raw'
              then jsonb_build_array(p_customer_changes ->> 'phone_raw') else '[]'::jsonb end
          ) where value is not null
      ) keys;
    foreach v_phone in array v_phone_keys loop
      perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(p_store_id::text || ':phone:' || v_phone, 0));
    end loop;
  end if;
  ...
  if p_customer_changes <> '{}'::jsonb and v_customer.updated_at is distinct from p_expected_customer_updated_at then
    return jsonb_build_object('ok', false, 'code', 'customer_stale_version');
  end if;
  -- Apply only submitted customer fields. In particular, omission of backups
  -- preserves them, while [] removes every backup without merging old numbers.
  ...
  if v_phone_changed then
    select coalesce(array_agg(phone order by ordinal), '{}'::text[]) into v_contacts from (
      select distinct on (raw) phone, ordinal from (
        select btrim(value) as phone, ordinal,
          pg_catalog.regexp_replace(value, '[^0-9]', '', 'g') as raw
          from unnest(v_customer.contact_phones) with ordinality phones(value, ordinal)
      ) normalized where raw <> '' and raw <> v_customer.phone_raw order by raw, ordinal
    ) deduplicated;
    v_customer.contact_phones := v_contacts;
    select array_agg(distinct phone_key) into v_phone_keys from (
      select v_customer.phone_raw as phone_key
      union all select pg_catalog.regexp_replace(value, '[^0-9]', '', 'g') from unnest(v_contacts) value
    ) keys where phone_key <> '';
    if exists (
      select 1 from public.customers c where c.store_id = p_store_id and c.id <> v_customer.id
        and (c.phone_raw = any(v_phone_keys) or exists (
          select 1 from unnest(c.contact_phones) phone
            where pg_catalog.regexp_replace(phone, '[^0-9]', '', 'g') = any(v_phone_keys)
        ))
    ) then return jsonb_build_object('ok', false, 'code', 'customer_phone_conflict'); end if;
  end if;
  -- Timestamp is assigned after lock waits, and must advance both versions.
  v_now := greatest(clock_timestamp(), v_order.updated_at + interval '1 microsecond', v_customer.updated_at + interval '1 microsecond');
  ...
  update public.repair_orders set
   ...
   returning updated_at into v_next.updated_at;
  if p_customer_changes <> '{}'::jsonb then
    update public.customers set name = v_customer.name, phone_e164 = v_customer.phone_e164,
      ...
      returning updated_at into v_customer.updated_at;
  end if;
  ...
  v_result := jsonb_build_object('ok', true, 'updated_at', v_next.updated_at,
    'customer_updated_at', v_customer.updated_at, 'replayed', false);
...
$$;
revoke all on function public.repairdesk_mutate_order_v4(uuid,uuid,uuid,timestamptz,uuid,text,text,jsonb,jsonb,timestamptz) from public, anon, authenticated;
grant execute on function public.repairdesk_mutate_order_v4(uuid,uuid,uuid,timestamptz,uuid,text,text,jsonb,jsonb,timestamptz) to service_role;

-- Preserve the old signature and its receipt authorization. A new customer
-- change cannot obtain a current version implicitly through this entry point.
...
as $legacy$
  select public.repairdesk_mutate_order_v4(
    p_store_id, p_actor_id, p_order_id, p_expected_updated_at, p_operation_id,
    p_request_hash, p_mode, p_order_changes, p_customer_changes, null
  );
$legacy$;
