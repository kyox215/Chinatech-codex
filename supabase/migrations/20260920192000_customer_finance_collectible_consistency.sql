-- Align customer-list receivables with the canonical order financial state.
-- Paid/refunded conflicts remain visible in history and quoted totals, but are
-- excluded from collection totals. No historical order amount is rewritten.
...
raw_order_facts as (
  select
    ...
    greatest(coalesce(ro.deposit_amount, 0), 0)::numeric as deposit_amount,
...
order_facts as (
  select
    ...
    raw_order_facts.is_valid
      ...
      and raw_order_facts.payment_status = case
        when raw_order_facts.deposit_amount > 0 then 'partial'
        else 'unpaid'
      end as is_collectible
...
comment on function public.repairdesk_customer_list_page_v3(
  ...
  integer,
  integer
) is 'Customer page v3: receivables exclude cancelled, voided, deleted, paid, refunded, and paid-balance conflict orders.';
