...
import { recognizeInventoryImageLocally } from "./inventory-local-recognition";
...
describe("local AI inventory recognition", () => {
  ...
  it("returns structured candidates from native local detectors without provider calls", async () => {
    ...
    const result = await recognizeInventoryImageLocally({ previewUrl: "blob:local-safe" });
  ...
  it("honors cancellation before decoding", async () => {
    ...
    controller.abort();
    await expect(
      recognizeInventoryImageLocally(
