export const inventoryImageMimeTypes = ["image/jpeg", "image/png", "image/webp"] as const;
export type InventoryImageMimeType = (typeof inventoryImageMimeTypes)[number];

export function detectInventoryImageMime(bytes: Uint8Array): InventoryImageMimeType | null {
...
export function isAnimatedInventoryImage(bytes: Uint8Array, mimeType: InventoryImageMimeType) {
  if (mimeType === "image/png") return pngHasAnimationControl(bytes);
  if (mimeType === "image/webp") return webpHasAnimation(bytes);
  return false;
}

export function readInventoryImageDimensions(bytes: Uint8Array, mimeType: InventoryImageMimeType) {
