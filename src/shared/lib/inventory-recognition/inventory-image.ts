import {
  ...
  inventoryImageMimeTypes,
  isAnimatedInventoryImage,
  readInventoryImageDimensions,
  type InventoryImageMimeType,
} from "./inventory-image-format";
import {
  INVENTORY_IMAGE_MAX_DERIVED_BYTES,
  INVENTORY_IMAGE_MAX_EDGE,
  INVENTORY_IMAGE_MAX_ORIGINAL_BYTES,
  INVENTORY_IMAGE_MAX_PIXELS,
...
export const INVENTORY_IMAGE_DATA_URL_TIMEOUT_MS = 8_000;
export const INVENTORY_CLIENT_PIPELINE_TIMEOUT_MS = 75_000;
export type InventoryImageErrorCode =
  | "empty"
  | "too_large"
  | "unsupported_type"
  | "mime_mismatch"
...
export class InventoryImageError extends Error {
  readonly code: InventoryImageErrorCode;

  constructor(code: InventoryImageErrorCode, message: string) {
    super(message);
    this.name = "InventoryImageError";
    this.code = code;
  }
}
...
export type InventoryNormalizedCrop = {
  x: number;
  y: number;
  width: number;
  height: number;
...
export type InventoryImageRuntime = {
...
export async function inspectInventoryImage(file: File) {
  if (file.size === 0) {
    throw new InventoryImageError("empty", "图片内容为空，请重新拍摄或选择图片。");
  }
  if (file.size > INVENTORY_IMAGE_MAX_ORIGINAL_BYTES) {
    throw new InventoryImageError("too_large", "原图不能超过 4 MiB，请裁剪标签区域后重试。");
  }

  const declaredMime = normalizeMimeType(file.type);
  if (!declaredMime) {
    throw new InventoryImageError("unsupported_type", "仅支持 JPEG、PNG 或 WebP 静态图片。");
  }

  const bytes = new Uint8Array(await file.arrayBuffer());
  const detectedMime = detectInventoryImageMime(bytes);
  if (!detectedMime) {
    throw new InventoryImageError(
      "unsupported_type",
  ...
  if (isAnimatedInventoryImage(bytes, detectedMime)) {
    throw new InventoryImageError("animated", "不支持动画图片，请上传单帧静态照片。");
  }

  const dimensions = readInventoryImageDimensions(bytes, detectedMime);
  if (!dimensions) {
    throw new InventoryImageError("dimensions", "无法安全读取图片尺寸，请重新导出为 JPG。");
  }
...
export async function prepareInventoryImage(
  file: File,
  runtime: InventoryImageRuntime = browserImageRuntime,
): Promise<PreparedInventoryImage> {
  const inspected = await inspectInventoryImage(file);
 ...
 */
export async function cropPreparedInventoryImage(
  prepared: PreparedInventoryImage,
  crop: InventoryNormalizedCrop,
  options: { signal?: AbortSignal; runtime?: InventoryImageRuntime } = {},
): Promise<PreparedInventoryImage> {
  ...
  try {
    ...
    if (decoded.width !== prepared.width || decoded.height !== prepared.height) {
      throw new InventoryImageError("dimensions", "规格裁剪尺寸校验失败，请重新选择图片。");
    }
...
function validateDecodedDimensionsMatch(
  ...
  if (!same && !orientationSwapped) {
    throw new InventoryImageError("dimensions", "图片声明尺寸与解码结果不一致，请重新导出为 JPG。");
  }
}

export function inventoryImageBlobToDataUrl(
  blob: Blob,
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    const timeoutMs = Math.max(
      1,
      Math.min(options.timeoutMs ?? INVENTORY_IMAGE_DATA_URL_TIMEOUT_MS, 60_000),
    );
    let settled = false;
    ...
    reader.onload = () => {
      ...
      if (value.startsWith("data:image/jpeg;base64,")) {
        ...
        finish(() =>
          reject(new InventoryImageError("processing_failed", "图片编码失败，请重试。")),
        );
      }
    };
    reader.onerror = () =>
      finish(() => reject(new InventoryImageError("processing_failed", "图片读取失败，请重试。")));
    reader.onabort = () =>
      finish(() =>
        reject(
          new InventoryImageError("processing_failed", "图片读取已取消，请重试或继续手工录入。"),
        ),
      );
    ...
    timeoutId = setTimeout(() => {
      finish(() =>
        reject(
          new InventoryImageError("processing_failed", "图片读取超时，请重试或继续手工录入。"),
        ),
      );
      abortReader();
    }, timeoutMs);
...
function normalizeMimeType(value: string): InventoryImageMimeType | null {
  const mimeType = value.trim().toLowerCase();
  return inventoryImageMimeTypes.find((candidate) => candidate === mimeType) ?? null;
}

function validateDecodedDimensions(width: number, height: number) {
  if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width <= 0 || height <= 0) {
    throw new InventoryImageError("dimensions", "图片尺寸无效，请重新拍摄。");
  }
  if (
    width > INVENTORY_IMAGE_MAX_EDGE ||
    height > INVENTORY_IMAGE_MAX_EDGE ||
    width * height > INVENTORY_IMAGE_MAX_PIXELS
  ) {
    throw new InventoryImageError(
      "dimensions",
...
async function encodeMetadataFreeJpeg(decoded: DecodedImage, runtime: InventoryImageRuntime) {
  ...
  for (let attempt = 0; attempt < 7; attempt += 1) {
    ...
    if (blob.size > 0 && blob.size <= INVENTORY_IMAGE_MAX_DERIVED_BYTES) {
  ...
  throw new InventoryImageError("derived_too_large", "图片压缩后仍然过大，请靠近标签重新拍摄。");
}

async function encodeMetadataFreeJpegCrop(
  decoded: DecodedImage,
  crop: InventoryNormalizedCrop,
  runtime: InventoryImageRuntime,
  signal?: AbortSignal,
) {
  ...
  throw new InventoryImageError("derived_too_large", "规格裁剪压缩后仍然过大，请缩小裁剪范围。");
}

function validateNormalizedCrop(crop: InventoryNormalizedCrop) {
  ...
  if (
    ...
    throw new InventoryImageError("dimensions", "规格裁剪范围无效，请重新调整后再预览。");
  }
}
...
const browserImageRuntime: InventoryImageRuntime = {
