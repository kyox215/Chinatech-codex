...
const sensitiveAuditKeyPatterns = [
  ...
  /authorization/i,
  /message[_-]?body/i,
  /issue[_-]?description/i,
  /diagnosis[_-]?result/i,
  /fault[_-]?prices/i,
  /cost[_-]?inputs/i,
  /cost[_-]?amount/i,
  /(^|[._-])cost$/i,
  ...
  /(^|[._-])gtin$/i,
  /display[_-]?value/i,
  /normalized[_-]?value/i,
  /document/i,
  /signature/i,
  /agreement[_-]?snapshot/i,
  /unlock/i,
  /address/i,
  /contact[_-]?phones/i,
  /file[_-]?name/i,
...
function sanitizeAuditValue(value: unknown, key = "", depth = 0): unknown {
  ...
  if (
    /^(?:input|cached_input|cache_write|output|total)_token_count$/.test(leafKey) &&
    typeof value === "number"
  ) {
    return value;
  }
