...
export function KioskSettingsSection({
  ...
  const deviceSummary = canManageDevices ? `${activeDevices.length} 台已授权` : "设备无管理权限";
  const reviewSummary = canReviewSessions ? `${submittedSessions.length} 项待审核` : "审核无权限";
  ...
  const createPairing = async () => {
    ...
    if (!label || pairingPendingRef.current) return;
    ...
    try {
      ...
      setPairingError(error instanceof Error ? error.message : "生成配对码失败");
    } finally {
...
function KioskPairingPanel({
  ...
  return (
    <div className="grid min-w-0 gap-3 rounded-xl border border-[var(--border-panel)] bg-card p-3 shadow-[var(--shadow-card)]">
      <div>
        <p className="text-sm font-semibold">设备配对</p>
      ...
      {canManage ? (
        <div className="grid gap-2">
          <Label htmlFor="kiosk-device-label">新 iPad 名称</Label>
          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
            ...
            <Button
              ...
              onClick={onCreate}
            >
              <Plus className="size-4" /> {pending ? "正在生成" : "生成配对码"}
            </Button>
          </div>
        </div>
      ) : (
        <PermissionNotice label="当前账号不能生成或撤销客户 iPad。" />
      )}
        ...
        <div className="grid min-w-0 gap-3 rounded-xl border border-primary/25 bg-primary/5 p-3">
          <div className="flex min-w-0 flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="text-xs font-medium text-primary">本次配对码</p>
              <p
...
function KioskDeviceList({
  ...
  return (
    <div className="grid min-w-0 gap-2">
      <SectionTitle label="设备列表" count={devices.length} />
      {!canManage ? (
        <PermissionNotice label="当前账号没有设备管理权限，页面不会请求或显示设备资料。" />
      ) : loading ? (
        <DomainSkeleton />
      ) : error ? (
        <DomainError label="设备读取失败" onRetry={onRetry} />
      ) : devices.length ? (
        <div className="grid min-w-0 gap-2">
          {devices.map((device) => (
            ...
            >
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                ...
                <Badge variant={device.status === "active" ? "default" : "outline"}>
                  {kioskDeviceStatusLabel(device)}
                </Badge>
              </div>
              <p className="mt-1 break-words text-xs text-muted-foreground">
                {device.last_seen_at
                  ? `最后连接 ${formatDateTime(device.last_seen_at)}`
                  : device.paired_at
        ...
        <EmptyBlock label="当前店铺暂无 iPad 设备" />
      )}
    </div>
  );
}
...
function KioskReviewQueue({
  ...
  return (
    <div className="grid min-w-0 gap-2">
      <SectionTitle label="待员工审核" count={sessions.length} important={sessions.length > 0} />
      {!canReview ? (
        <PermissionNotice label="当前账号没有客户提交审核权限，页面不会请求或显示提交内容。" />
      ) : loading ? (
        <DomainSkeleton />
      ) : error ? (
        <DomainError label="审核任务读取失败" onRetry={onRetry} />
      ) : sessions.length ? (
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 grid min-w-0 gap-2 sm:grid-cols-2">
        ...
        <KioskReviewField label="备注" value={note} icon={MessageSquare} wide />
      </div>

      <div className="mt-3 grid gap-1.5">
        <div className="flex items-center justify-between gap-2">
          <Label htmlFor={`kiosk-return-${session.id}`}>给客户的退回原因</Label>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        ...
        >
          <RotateCcw className="size-4" /> 退回重填
        </Button>
        <Button
          type="button"
          className="min-h-11"
...
function KioskRecentActivity({
  ...
  return (
    <div className="grid min-w-0 gap-2">
      <SectionTitle label="最近处理" count={sessions.length} />
      {sessions.length ? (
        <div className="grid min-w-0 gap-2 sm:grid-cols-2">
          {sessions.map((session) => (
            ...
            >
              ...
              <p className="mt-1 break-words text-xs text-muted-foreground">
                {session.device?.label ?? "客户 iPad"} · {formatDateTime(session.updated_at)}
              </p>
            </div>
          ))}
        </div>
      ) : (
        <EmptyBlock label="暂无最近处理记录" />
      )}
    </div>
  );
}
...
function KioskActionConfirm({
  ...
  const description =
    target?.kind === "revoke"
      ...
      : target?.kind === "return"
        ? `客户会看到退回原因：“${target.reason}”。提交内容不会在这里静默修改。`
    ...
    <AlertDialog open={Boolean(target)} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          >
            {pending ? "正在处理" : target?.kind === "revoke" ? "确认撤销" : "确认提交"}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
...
function DomainError({ label, onRetry }: { label: string; onRetry: () => void }) {
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs">另一类 iPad 数据不受影响。</p>
    </RepairOsBusinessCard>
  );
}
...
function usePairingCountdown(expiresAt?: string) {
  ...
  return remaining > 0 ? `${minutes}:${String(seconds).padStart(2, "0")}` : "已过期";
}
...
function kioskDeviceStatusLabel(device: KioskDevice) {
  if (
