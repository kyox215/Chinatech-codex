  ...
  return (
    ...
    >
      <RepairOsSectionHeader
        ...
        iconFrame={false}
        title={copy("关闭与删除店铺")}
        description={copy("店铺删除分两步：先安全关闭（可恢复），归档后才可以申请永久删除。")}
      />
      <p className="text-xs leading-5 text-muted-foreground">
        {copy("当前店铺")}：<span className="font-semibold text-foreground">{store.name}</span>。
        {copy("安全检查只读取状态，不会立刻删除资料。")}
      </p>
