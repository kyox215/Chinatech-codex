...
describe("store repository access request boundaries", () => {
  ...
  it("publishes cost export only with the independent child flag and grant", async () => {
    ...
    const grantedManager = {
      ...storeManager,
      permissionGrants: ["finance:profit_read" as const, "finance:cost_export" as const],
    };
    const [ownerContext, managerContext, grantedManagerContext, technicianContext] =
      await Promise.all([
        getStoreContext(storeOwner),
        getStoreContext(storeManager),
        getStoreContext(grantedManager),
        getStoreContext({ ...storeTechnician, permissionGrants: ["finance:cost_export"] }),
      ]);

    expect(ownerContext.permissions).toMatchObject({ canExportRepairCosts: true });
    expect(managerContext.permissions).toMatchObject({ canExportRepairCosts: false });
    expect(grantedManagerContext.permissions).toMatchObject({ canExportRepairCosts: true });
