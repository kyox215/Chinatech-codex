      ...
      <div className="min-w-0 space-y-2">
        {followups.length ? (
          followups.map((item) => (
            <div
              key={item.id}
              className="flex min-w-0 flex-col gap-2 rounded-2xl border border-[var(--border-panel)] bg-card px-3 py-2 shadow-[var(--shadow-card)] sm:flex-row sm:items-center sm:justify-between"
            >
...
export function CustomerTimelinePanel({ data }: { data: CustomerDetail }) {
  return (
    <section className={cn(repairOs.adminSection, "min-w-0 p-2.5 sm:p-3")}>
      <h2 className="mb-2.5 text-sm font-semibold">客户时间线</h2>
