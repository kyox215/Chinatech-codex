...
function orderPlannerInstructions(locale: AiAssistantLocale) {
  return [
    ...
    "For 上周/last week/settimana scorsa, emit previous_calendar_week; for 本月/this month/questo mese, emit current_calendar_month.",
    "For rolling periods such as 半年内 or last 45 days, emit rolling_period with the exact amount and unit. For explicit local calendar dates, emit absolute_range with YYYY-MM-DD endpoints. For all dates, emit all_time.",
    "Never replace an unsupported or ambiguous date with a different period. Never calculate UTC timestamps.",
    "Do not add payment, finance, status, queue, parts, service, date, view, or completion filters unless the user explicitly requested them.",
    "Use completed_at and completed_only=true only when the user explicitly asks for completed or repaired-finished orders; otherwise date queries use created_at.",
