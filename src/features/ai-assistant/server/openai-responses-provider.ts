...
const orderPlannerTools = [
  {
    ...
    name: "search_orders",
    description:
      "Search the current authorized RepairDesk store using bounded structured filters. Preserve a concrete device brand/model in device_search. Represent relative dates symbolically in date_filter so the server resolves the store calendar. service_group matches quote catalog evidence only, never proof of performed work. parts_status is an order-level marker, never a supplier purchase order. Use search only for a concrete order/customer term; never put abstract concepts into search.",
    strict: true,
...
function orderPlannerInstructions(locale: AiAssistantLocale) {
  return [
    ...
    "For 上周/last week/settimana scorsa, emit previous_calendar_week; for 本月/this month/questo mese, emit current_calendar_month. Never calculate timestamps.",
