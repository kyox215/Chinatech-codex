...
[Global content editing standard](GLOBAL_CONTENT_EDITING_STANDARD.md) is active. `mobileEditor` is an explicit opt-in on DialogContent/SheetContent; `editorSurface` alone never repositions a modal. Instruction14 keeps repair categories in a 4×3 grid with4px gaps and fixed36px single-line cells. The left two-thirds selects; the right third opens the full group label and options. Save/Cancel remain44px, inputs remain16px, and existing inventory selector/capture target requirements remain unchanged. Preserve one focus owner, stable draft sessions and existing feature guards.
...
Grouped order editors reuse the additive `componentOverlay.denseEditor*` slots for a 52px header, 8–10px rhythm and 44px close/save controls. Instruction15 makes `OrderWorkspaceQuoteTextField` and `OrderWorkspaceQuoteDisclosure` compact button triggers for one shared quote popup, using existing DialogContent `mobileEditor`/`editorLayout`, scrolling body and separate footer. No inline textarea/details expansion, resize observer or window Escape interception remains. Each callsite preserves its prior editability: existing editable names (including catalog-backed names) and specifications use a local popup textarea, single-line stored value and IME-safe Enter that saves only the parent draft. Existing display-only disclosures remain read-only; editing a catalog-backed name retains the original catalog-key clearing callback. Cancel/close discard local changes, opening focuses the container, and closing restores the trigger without reopening. Quote popup triggers opt into `data-keypad-defer-dismiss`: VirtualKeyboardDock keeps their position through pointerdown and dismisses without restoring money focus after the same click. Unmarked controls retain pointerdown dismissal; cancelled gestures do not open a popup. Amount/removal remain independently aligned; money rules, global guards and existing unrelated controls are unchanged.
