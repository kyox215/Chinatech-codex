...
Grouped order editors use the additive `componentOverlay.denseEditor*` slots for a52px header,8–10px rhythm and44px close/save controls. `denseInlineField` supplies the enclosing focus ring; unrelated dialogs/Input defaults remain unchanged. Reuse `AccessoryNotesPicker` for existing none/custom/disabled logic. Instruction14 makes quote names/specifications compact by default: `OrderWorkspaceQuoteTextField` keeps one single-line textarea and expands it only through its explicit toggle or user editing focus; Escape collapses and restores toggle focus. `OrderWorkspaceQuoteDisclosure` uses native details/summary for read-only text, with keyboard activation and Escape. No duplicate editor is mounted. Price/removal retain aligned independent columns, validation a separate track, and three-column money summaries preserve whole numeric tokens. IME, single-line stored values, implicit submission and deferred ResizeObserver writes remain intact.
