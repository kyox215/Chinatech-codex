...
桌面端显示固定侧边栏，支持折叠为纯图标模式；移动端不再显示底部导航，而是使用同一套侧边栏抽屉，默认收纳，由 AppBar 左侧菜单按钮打开。进入模块后，页面内部不再重复显示大号“工作台 / ...”“订单管理 / 回收管理 / 库存商品”“全部 · 共 ...”等标题区，首屏直接进入指标、筛选和业务卡片。扫码是全局工具，不进入导航。悬浮 `+` 根据当前模块显示高频动作，例如新建工单、新建回收、库存入库、客户操作、扫码读取、拍照采集。
...
状态：默认收纳到筛选入口；仅核心工作队列使用少量换行 chips、固定网格或小 KPI，订单工作队列禁止横向滑动状态栏
主体：高密度业务卡片列表
导航：桌面固定侧边栏 / 移动侧边栏抽屉
右下：模块化快捷 +
```

桌面端保留侧栏、AppBar 面包屑和高密度表格；移动端保留顶部菜单入口、搜索/店铺上下文和模块化 `+`，页面内容从指标/工具条开始。新增页面不得在正文里再生成模块标题块，必须先保证 390px 宽度可用。
...
- 移动端没有横向溢出。
- 主色切换为 `#315CFF`。
- 新页面使用 RepairOS Compact 卡片；默认不添加顶部状态 chips，只有产品确认的核心工作队列例外。
