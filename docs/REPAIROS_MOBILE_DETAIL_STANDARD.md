...
订单详情与新建报价共用 12 类三列分体选择器：左侧标题保留目录快捷行为，右侧箭头打开维修/检查菜单，选中同时显示主色与勾选。保持 44px 分类/编辑/主要操作目标，真实手机输入至少 16px；卡内使用 8–10px 间距、短字段并排，不叠加内卡或明细内滚动。

报价金额与定金显式选择 `MoneyKeypadInput` 的 `keyboardMode="native"`，复用既有 decimal normalizer 和保存值；其余调用仍遵循原有键盘偏好。明细按名称、欧元价格、删除排列；新建目录名称只读，自定义名称可编辑。总额、尾款、定金同排，计价及创建载荷不变。

详情报价卡头“收起分类 / 选择项目”只切换分类可见性，草稿与底部保存/取消始终保留。人员/供应商“收起”仅折叠已存在的即时编辑面板。移动详情底部核心动作至少 44px，并沿用原底部避让；历史记录继续使用顶部页签，不增加重复入口。
