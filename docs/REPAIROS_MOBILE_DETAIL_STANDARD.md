...
- 客户症状、技师检测发现、维修/报价项目必须分区展示，不把同一个 code 同时当症状和维修项目。
- 常见项使用至少 44px 高的多选 chip；只有“其他”出现文字输入。
- 历史文本不反向猜 code；旧目录或停用 code 恢复时显示待复核，不静默替换。
- 初始定金不在通用编辑表单直接修改。允许更正时使用独立金额键盘 + 预设原因弹层；已有审批或收款证据时隐藏入口并引导到付款流水。
- 返修接收判断与检测后处置分开；接收判断不得自动承诺免费、保修责任或已创建关联新单。
