...
- 桌面首个API请求、详情文档标记与打印数据准备统一等待应用runtime、CSS就绪信号和恢复遮罩结束。真实请求、文档身份、草稿和打印断言保留；冷启动与三项国际化失败用例定向通过。
- 回收国际化fixture仅增加同源GET/HEAD `/recovery-probe.txt` 为合法静态读取，仍阻止未知API、越权写入及外部请求。
- 打印进度截图在CI耗时2.6秒，超过产品成功提示的2秒寿命；测试改用可释放的二维码响应，先截图再释放响应并检查成功提示。Chromium/WebKit四种PDF模式各定向通过，产品计时未修改。
- 44px关闭按钮在动画中被boundingBox读成43.99994px；改验实际CSS高度44px。权限敏感确认、仅指定端点提交、错误恢复等断言仍在，该完整用例定向通过。
- 上一轮桌面63通过；交互76通过+1上述尺寸误差失败；国际化分组原始日志与失败trace保留。新提交仍须通过完整云端检查，不能用这些分批结果冒称已发布。
