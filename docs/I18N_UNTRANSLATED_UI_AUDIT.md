...
Baseline commit: `fdeb7b13e8c9757911bd4e21f37c34497ed6941e`

## 结论

仓库基线包含 **5,839 次 Han-script 出现、4,213 个唯一候选**，但这个数字是待分类的审计输入，不是 5,839 个翻译缺陷。至少 935 次来自明确的旧实现或非生产路由；测试、Story/mock、中文源 catalog、canonical code 和动态业务数据也不应机械翻译。
...
脚本读取 TypeScript 配置，用 TypeScript AST 检查生产 `src/**/*.tsx` 中的 JSX 文本、JSX attribute 字符串和普通字符串；它排除 test/spec/story、server、testing、API 路径及消息 catalog。基线结果为：
...
当前实现复扫为 **5,599 次出现、4,088 个唯一候选**，恰好减少 240 次；上述 16 个 TSX consumer 的直接 Han-script 候选为零。这个差值只证明冻结的直接字面量已迁移，运行时 adapter 仍需单独测试，不能以数字替代验收。
