...
- Order Query V3 只允许模型规划 `search_orders`、`get_order_summary` 与安全澄清三类只读意图；`actor`、当前门店、日期范围、权限和可操作项始终由服务端注入。
...
- 用户显式选择“大模型辅助”时仍会真实调用现有受控 provider，但模型生成的过滤器只是一份候选计划。最终查询采用 closed-world 编译：服务端从原句独立重建所有允许执行的限制条件，模型未获原句支持的 Samsung、金额异常、付款、状态或其他范围会被完整丢弃。原句没有任何可验证条件时，即使模型返回搜索或摘要也只会请求澄清，不访问订单仓库。严格 tool schema 保证结构，不替代应用层语义校验，参见 [OpenAI Structured Outputs](https://developers.openai.com/api/docs/guides/structured-outputs)。
- 结果卡显示理解状态、实际范围、精确 `from/to`、日期字段、`Europe/Rome` 口径和条件来源。普通已确认范围默认折叠；纠正、默认、权限限制或零结果默认展开。零结果与说明合并为一张卡，部分结果明确显示“当前 N / 共 M”，修改查询只回填输入框，不自动发送。
...
再运行 `tests/e2e/ai-assistant-staff.spec.ts` 的 fake-provider 流程，覆盖桌面、390/430px、金额异常本地查询、显式大模型“检查半年内所有的苹果15系列的手机”、紧凑处理/用量折叠、实际采用条件、卡片内详情、显式订单链接、取消、离线、权限拒绝、敏感输入提示和手机快捷入口顺序。写操作只用合成数据验证确认、乐观锁、权限与幂等；生产旗标关闭时不得执行写 smoke。真实计费 read smoke 必须在 durable 数据库政策启用后，用一条无 PII 的合成查询执行一次；不得绕过预算网关直接调用 provider。
