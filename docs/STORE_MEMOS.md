...
Checklist update: 2026-09-26（本地实施；生产迁移与发布另行批准）
...
页面支持全文搜索、类型、负责人和查看范围筛选，以及分页。查看范围包含当前记录、待处理、我的、
逾期、已完成和已归档，但统一收纳在筛选面板，不在页面顶部永久平铺。列表使用紧凑行；手机使用底部 Sheet，iPad和桌面使用 Dialog。编辑器保留未保存草稿保护，并在版本变化时阻止覆盖保存。

## 型号子清单

待办可包含最多100个子项，每项为稳定UUID、最多200字的文字、完成状态；正文仍是独立备注。
例如标题“手机壳”，子项“iPhone 17 Pro 透明壳 ×3”。数量仅为自由文字，不产生库存或采购操作。

- 首页默认显示进度，展开后可直接勾选并保存；完成项单独折叠，搜索命中的完成项可查看。
- 增删和文字修改在编辑器中统一保存，多行粘贴忽略空行并保持顺序，超限提示且不截断。
- 全部子项完成时整条自动完成；取消一个勾选或新增未完成项自动重开。
- 有子项时禁止整条手工完成/重开；删除最后一项保留之前整条状态，恢复普通待办操作。
- 普通记录不支持清单；旧待办默认空清单，原内容不自动解析、合并或转换。
- 改清单文字沿用编辑权限，逐项勾选沿用状态变更权限；被分配员工可以完成但不能借此改写他人内容。
- 置顶、复制模板、通知提醒与库存联动不在本次范围。

## HTTP API

所有端点均为 `POST /api/repairdesk/<path>`，复用现有登录、当前店铺解析、CSRF、私有响应和
错误映射。读取同样要求功能已对当前店铺开放。
备忘录直接使用登录态解析出的当前有效店铺；单店账号使用唯一店铺，多店账号使用当前登录或切换
后的店铺，不要求额外确认。所有读取和写入仍由服务端以 actor 的 `store_id`、有效 membership 和角色
权限强制限定，不能跨店访问。
...
| `memos/checklist-item` | `{ input: { operationId, id, expectedVersion, itemId, completed } }` | 原子保存子项并返回完整 mutation envelope |
| `memos/transition` | `{ input }`                             | 领取、完成或重开后的记录        |
| `memos/archive`    | `{ input }`                             | 归档后的记录                    |
| `memos/restore`    | `{ input }`                             | 恢复后的记录                    |

写请求必须包含 UUID `operationId`；更新、状态与归档操作还必须包含 `expectedVersion`。服务端创建/编辑请求体上限为128 KiB，其他备忘录接口保持8 KiB。数据库保存请求哈希与结果引用：相同操作号和相同载荷安全重放，不同载荷
...
完整正文和清单。列表仅携带 `checklist_total` / `checklist_completed` 汇总，搜索在服务端匹配标题、正文和子项文字。分页最大 100 页，版本过期返回 HTTP 409。

创建/编辑增加可选 `checklist: { id, text, completed }[]`；更新省略该字段保留已存数据，显式 `[]` 清空。逐项写入使用目标布尔值而不是 toggle，重复请求不会反转。失败保留旧状态并允许重试，冲突不得自动覆盖草稿。
...
1. 先完成增量迁移及新旧RPC兼容性验证，核对目标项目、迁移历史、备份和正式发布授权。
2. 新字段默认空清单；新增RPC对清单和整条状态统一进行版本、幂等、权限及审计处理。旧调用省略清单不清空新数据，旧整条完成/重开请求不得绕过清单汇总。
3. 迁移先于应用；未迁移的数据库不应运行新应用。观察403/409/422/429、写延迟、搜索、revision与广播失败。
4. 回退应用时保留新增字段、清单和receipt，不执行丢数据的down migration。旧应用不能操作子项，必要时停用备忘录功能等待修复。
5. 本地回滚事务测试与mock浏览器测试不等于生产迁移、生产双设备同步或备份恢复演练；实际证据见当前任务 EVIDENCE。
