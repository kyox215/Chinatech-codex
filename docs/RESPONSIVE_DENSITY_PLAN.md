...
新建工单工作区采用“报价为主、报障为辅”的稳定布局：

- `>= 1280px` 使用三个互不耦合的纵向区域：客户/设备在左、报价固定在中、客户报障与手机密码在右。右栏内部自行堆叠，不允许通过跨行 Grid 让中栏高度撑大右栏间距。
- `1024px–1279px` 为左右两栏：客户/设备在左，右栏按报价、客户报障、手机密码紧密堆叠；`< 1024px` 保持同一 DOM/键盘顺序并降为单栏。
- 客户报障主卡只显示两档模式、单行摘要和编辑入口；详细原话在移动端 Sheet、桌面端 Dialog 中编辑。切换模式不得改变主卡或报价卡几何尺寸。
- “待检测”只暂停报价提交，不卸载报价控件或删除草稿。客户原话、报价项目与定金可留在有期限的本机草稿中，但在线创建与离线 outbox 必须解析为标准待检测描述、空报价项目和零定金；本机专用暂停字段进入 outbox 前必须剥离。
- 响应式门禁至少覆盖 `390 / 430 / 768 / 1024 / 1280 / 1440`，断言页面无横向溢出，且模式切换前后客户报障卡与报价卡的 `x / y / width / height` 变化不超过 1px。
...
- 搜索、分组、更多筛选和页码写入 URL，进入详情再返回时恢复原列表上下文。
