...
Follow [GLOBAL_CONTENT_EDITING_STANDARD.md](GLOBAL_CONTENT_EDITING_STANDARD.md). Order detail/new quote categories retain exactly 4×3 cells and 4px gaps. Instruction 12 replaces the fixed 33px height with a 36px minimum and natural growth for complete multilingual labels. Compact inputs remain 16px and primary actions 44px. Inline assignment/finance expansion is replaced by stable bottom editing; preserve parent page position, independent desktop presentation and last-field/footer reachability at compressed heights.
...
For `quote-editor`, the name and specification occupy successive lines in the first column and grow without clipping. Editable names/specifications use the shared growing text field; price/remove retain independent first-row columns and amount errors occupy a secondary track. Total/deposit/balance remain three equal-height columns with content-driven height. New and detail entry points share these primitives. Category labels wrap inside their unchanged 2:1 selection/disclosure split; options remain scrollable through the last item. Independently verify zh/it/en at 320/390/430/768 and desktop 1024/1280/1440; include 320×350 close/footer/keypad reachability. Density is never a reason to drop an existing field or state.
