...
- 按2026-09-13用户批准的全屏工作台方案，工单、客户和商品列表与详情保持独立层级；已迁移的完整编辑任务采用独立页面或显式全屏工作区，不再默认从列表打开详情弹窗。短选择器、危险确认及系统打印保留各自尺寸与业务合同，不能全局放大所有弹窗。
...
### 工单首页 V2（2026-09-07，已接受方案的真实页面集成）

- `<1024px` 使用独立紧凑卡片与 RepairOS 浮卡页头：手机号优先，其次设备和维修摘要，右侧集中状态与金额；姓名等于手机号时不重复。一般卡片保持紧凑，异常、逾期、外协和特殊保管状态允许自然增高，完整长内容仍在真实详情查看。
- 首屏收起七个队列（全部及六个真实业务队列），通过当前队列与结果数打开队列面板；全部计数和选中态在面板完整呈现。这样节省首屏空间，代价是切换队列多一次展开。数据范围 `active/archive/all` 独立显示与选择，归档权限和跨范围精确搜索保持原契约，不能用欠款重定义归档。
- 手机本页菜单、新建、搜索、扫码、筛选、队列、范围及弹层关闭/底部动作采用至少 `44px` 目标；队列选项至少 `52px`。这是已接受的工单首页局部契约，不改变上文通用密度分级。队列、范围、筛选复用共享 Dialog/滚动正文/底部安全区，打开聚焦容器，Escape 关闭并回到触发控件。
- 排序沿用真实全量契约：队列、流程进度、创建时间；只显示当前规则说明，不提供服务端尚不支持的“最近/姓名”选项，也不只排序当前页。
- 卡片和桌面行复用 `OrderMiniProgress` 的无文字 `h-1` 五段条及既有 `getWorkflowProgressValue`/`orderTaskStages` 映射。六队列不是流程步骤；终态和异常仍由真实流程模型判定。
- `>=1024px` 保持独立工作台：搜索和主动作在首行，完整队列名称在下一行，批量选择、打印/导出权限、技师信息与行详情弹窗沿用原机制。默认店内保管和 IMEI 不占用常驻行高度，完整信息仍在详情可查。
- 财务遮罩同时隐藏维修/报价项目与金额；取消工单显示记录金额而非待收金额。此次不修改 API、查询键、分页、实时同步、权限和创建/支付业务规则；工单二维码继续只识别可信工单码。
- 验收覆盖 `390/430/768/1024/1280/1440`，包含长意大利语、财务受限、队列竞态/失败回滚/离线、焦点与真实创建/详情入口；发布前仍需最终全量门禁。
...
Compact order detail has equal-width Details / Device photos / History tabs; desktop keeps its independent workspace tabs. The fault editor uses Sheet below 1024 and modalLg Dialog at desktop widths, visible fault and diagnosis textareas at 16px, a scrolling body and a padded safe-area footer. Timeline and message text remain fully readable rather than clipped.

### Small editor responsive contract (2026-09-05)

Fault editing uses an independent bottom Sheet at 390/430/768 and an 860px desktop Dialog at 1024/1280/1440 with a 240px reference column. Desktop footer state stays left while Cancel/Save use intrinsic widths on the right; mobile actions share two columns. Both edit and inline-confirmation steps preserve bottom safe-area padding. The existing desktop order-list workspace hosts the editor as an inline step within its one modal, retaining the hidden detail body and restoring its edit trigger. Owner-selected A now opts related customer/settings editors into mobile bottom presentation while preserving desktop orientation; memo already uses independent mobile/desktop surfaces; long forms must prove last-field and footer reachability, not fit every field into one screenshot.
