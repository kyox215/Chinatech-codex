...
- 路由：`@tanstack/react-router` 文件路由，文件位于 `src/routes/`，禁止引入 Next.js `page.tsx` / `middleware.ts` 模式。
...
根布局已在 `src/routes/__root.tsx` 固定：
...
新增页面只写 `<Outlet />` 内部内容，不重复创建侧栏、顶栏、背景、Toast 或 QueryClient。
...
3. 新路由必须提供 `head()`，至少包括 `title` 和 `description`。
4. 新导航页必须同步更新 `AppSidebar`、`AppBar` 的 `labels`、`CommandPalette`。
5. 不编辑 `src/routeTree.gen.ts`。
6. 不在页面内重复包 `AnimatePresence` 做路由切换；根布局已经处理。
...
export const Route = createFileRoute("/example")({
  head: () => ({
    meta: [{ title: "示例 — RepairDesk" }, { name: "description", content: "示例页面说明" }],
  }),
