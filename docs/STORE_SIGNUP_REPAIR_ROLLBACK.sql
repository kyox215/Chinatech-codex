-- INERT ROLLBACK ARTIFACT — DO NOT AUTO-APPLY.
--
-- This documentation-only artifact records the exact pre-forward function
-- definition, ACL, and PostgREST schema reload statement. It is not a
-- migration and must not be executed directly. A future, separately reviewed
-- forward migration may copy this exact body for a staging-only
-- forward -> rollback -> forward rehearsal after explicit authorization.
-- It contains no table/data/backfill/policy statements.
--
