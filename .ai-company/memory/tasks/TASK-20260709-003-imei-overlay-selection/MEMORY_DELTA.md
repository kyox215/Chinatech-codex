...
- When multiple barcode boxes are shown in one captured image, label candidates by visual top-to-bottom position rather than detector return order; detector order can differ from what the user sees. Source: `orderCandidatesByVisualPosition` in `src/components/imei-scanner-field.tsx`. Status: validated. Owner: frontend/UX. Review trigger: future multi-candidate capture changes.
