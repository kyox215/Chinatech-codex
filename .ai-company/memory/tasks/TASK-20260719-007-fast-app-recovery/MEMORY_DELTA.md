...
- The double-readiness and standalone-fallback contracts are production-verified and may be promoted to project/Architecture/Frontend/QA memory. The exact physical-iPhone background/BFCache/network-switch timing remains observed only and must stay in owned backlog.
...
- A linked Supabase application step with 91/91 parity and an up-to-date dry-run is a successful zero-write release result; never replay historical SQL to manufacture a migration action.
