...
- Production Vercel/Supabase state, live migration parity, staff/platform roles, backup/restore, and retention policies are not verified by local repo inspection.
...
- Latest completed task: `TASK-20260704-009-independent-partner-store-platform` independent partner-store Phase 1 local baseline. SG0-SG7 passed locally; Phase 2 tenant isolation audit is ready to start. This is not production release approval.
...
- 30-day focus: start Phase 2 tenant isolation audit, preserve production migration preflights/runbooks, Owner decision on role-policy Option A, permission denial test plan if approved, duplicate-file cleanup approval, stale-doc audit, audit sanitizer implementation if approved, production readiness checklist, legacy route cleanup approval, and repeatable health-check runbook.
