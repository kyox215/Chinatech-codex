...
- Active `TASK-20260718-011-ai-assistant-cost-governance` defines the Phase 3A dormant cost-governance extension. Capability/RBAC and an all-request abuse guard remain ahead of a conservative deterministic order planner; deterministic hits use no provider quota. Complete local label candidates may skip cloud upload but remain reviewed, unsaved packaging claims. Fixed runtime/model snapshots, HMAC safety identifiers, integer micro-USD estimates and a durable atomic quota migration are verified as default-off code/local schema evidence only. The migration is unapplied, OpenAI provider remains fail-closed, and all budget/privacy/key/live flags remain D4 decisions.
