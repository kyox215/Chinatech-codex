...
- Local-verified `TASK-20260723-004-startup-bootstrap-print-implementation` defines the current app-shell startup and print-readiness contract: one `shell/bootstrap` request hydrates onboarding, store context and AI capability, while 404/405/501 retain legacy compatibility and 401/403 fail closed without fallback. Home routes suppress cross-domain startup preload. Single-order print capability is distinct from owner-only batch print/export, and every list/detail print surface must expose an exact disabled reason plus recovery entry. The local suite passed 342 files / 2289 tests and build; commit, push and production deployment remain separate Owner decisions because the working tree contains concurrent task changes.
...
- Closed `TASK-20260720-001-customer-simple-workbench` defines the current customer workbench UI contract: four URL-backed list groups, one lower-frequency filter surface, five detail groups, and three primary actions. Below `lg`, fixed detail controls must avoid the desktop sidebar; external messaging records contact only after employee confirmation. Customer list responses remove list-unneeded sensitive fields after filtering/finance projection. This app-only release does not clear the linked migration-history NO-GO.
...
- Conditional `TASK-20260720-006-ai-ledger-fence-hotfix` repairs the production incompatibility introduced when the generic store writer fence rejected legal null-store global AI buckets. Applied migration `20260720065246` uses a table-specific mixed-scope trigger and serializes lifecycle exit with unsettled reservations. Catalog/ACL/RLS/aggregate checks, one non-PII order-text canary at 130 micro-USD and 15 minutes / 16 zero-threshold polls passed. Production functionality is restored; the remote hotfix branch must be integrated into `main` before the next database release.
- Conditional `TASK-20260719-008-inventory-imei-one-capture` supersedes only that Vision/photo boundary for the approved Chinatech Inventory V2 slice: `main@facb79b984de5ffdc596210cd9ba33883343053e` / READY `dpl_3HZsEL9XraLy1McLeaTxHCwsxpKs` performs complete-label barcode/OCR/IMEI recognition locally with same-origin fixed Workers. Full labels, raw OCR and identifiers cannot enter the cloud path; only a separate metadata-free specifications crop may be sent after explicit employee review, and all candidates remain unsaved. The release changed no schema, policy, budget, model, quota or allowlist; Supabase stayed 91/91 up to date with zero applied SQL. Production asset/log/zero-use/zero-write gates passed, but authenticated Chinatech mobile/desktop smoke remains open because the authorized test account belongs only to `xutech`. PII beyond the reviewed crop, automatic writes, public/customer AI, other stores and policy changes remain R4/D4 blocked.
...
## Store signup repair closeout (2026-08-27)

`TASK-20260823-002-store-signup-repair` is closed after production deployment `dpl_8Vk8CvGq4Bzj3ziaYkqFXk3Ki9QM` reached READY on runtime commit `e17434e8388959e14e8ed1de8323172e28c2c876` and the final authenticated read-only 60-minute observation gate passed. The committed migration ledger is `120` rows with `120` unique versions. The pure read-only postcheck was 320 lines, SHA-256 `f0fd3415ad343100524148e42e588b656efd1b168541689100f24c9314df7153`, execution `PASS` (`isError=false`), and its ledger/RPC/ACL/18-trigger/6-event/synthetic-residue/advisory-lock invariants passed. Sanitized Vercel target categories (`401`/`403`/`5xx`/timeout/`Invalid API key`/`STORE_CREATE`) were all `0`; post-READY Supabase observations were API `34`, Auth `14`, Postgres `2`, with target categories `0`. No real create-store POST, new account/store, or real multi-account E2E was executed or claimed. `PITR=false` remains an open recovery risk; future database recovery is forward-only and any new implementation, staging E2E, credential rotation, or recovery requires a new Owner-approved task.
