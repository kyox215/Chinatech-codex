...
- In the AI assistant, usage details and processing/privacy help are independent disclosures that default collapsed for a new sheet session. Their 44px trigger rows must keep the current mode visible; model mode must still state external OpenAI sending and usage charging while collapsed. Expanding one disclosure must not force the other open or change the selected mode.
