...
- In the AI assistant, processing mode and today's model usage share one default-collapsed composer row that keeps the current mode and compact request/limit usage visible. Its disclosure must remain keyboard-operable and preserve the selected mode; expanded content carries the local/model choices plus privacy and charging meaning. Result-card body expansion never navigates—only the explicit `打开订单` link may leave the conversation.
