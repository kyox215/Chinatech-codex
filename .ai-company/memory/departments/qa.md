...
- Cross-session coordination gates must cover exactly-one CAS outcomes, command replay mismatch, crash before/after commit, corrupt/locked Registry, symlink/runtime permissions, task/run/window/worker/WP mismatch, scoped lease closure, stale/tampered/orphan Context Packets, ACTIVE_CONTEXT isolation, malformed config fallback and secret/PII canaries. Rebase on the latest remote and rerun the complete suite before non-force push.
...
| QA-20260718-002 | Broad desktop E2E retains seven stale buyback/inventory dialog locators and one 1024px order-print locator failure reproducible on an untouched baseline | A broad-suite red result can hide release-unit truth or become normalized without ownership | QA + Buyback + Inventory + Orders | repair locators in a dedicated E2E maintenance task; continue baseline reproduction plus focused formal suites until then | baseline_debt_open |
...
| 2026-07-18 | Verified Phase 3A with 296 files / 1858 tests, Webpack build, 6/6 staff E2E and all four inventory behaviors; retained the no-business-request Next dev hydration limitation and clean-server rerun instead of hiding it | TASK-20260718-011-ai-assistant-cost-governance | QA/Release reviewer + Integration Lead | scoped_verified_dormant |
| 2026-07-18 | Verified three release units with 1803 tests, formal settings 67/67, custody 3/3, six responsive screenshots, exact baseline attribution and final production smoke | TASK-20260718-012-workspace-integration-release | QA/Release reviewer + Integration Lead | scoped_verified |
| 2026-07-18 | Verified 46 cross-session identity, concurrency, crash, recovery, permission, packet and compatibility tests plus three independent GO reviews | TASK-20260718-013-cross-session-orchestration-implementation | QA/Security reviewer + Integration Lead | scoped_verified_shadow |
