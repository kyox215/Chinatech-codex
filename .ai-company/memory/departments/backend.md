...
- `TASK-20260718-014-ai-assistant-live-pilot` activates only the existing employee order-read service for ChinaTech. Provider fallback is server-only, uses explicit `reasoning.effort=minimal`, reserves durable budget before the single attempt, validates usage before finalization, and writes aggregate privacy-safe audit metadata. No AI write tool, raw prompt/output audit, Vision or public route is authorized.
...
| Live AI order-read service | Architecture + Data + Security + QA | Employee RBAC/store context, ChinaTech allowlist, deterministic-first routing, v2 one-attempt provider and durable ledger/audit | Reject config/policy/tenant mismatch before dispatch; never retry; manual order search remains fallback                 | TASK-20260718-014 E-031–E-037                                         | scoped_verified_single_store |
...
| 2026-07-18 | Released capability-first request guard, conservative deterministic order routing, fixed provider signal/runtime contracts and aggregate cost audit fields with provider still fail-closed | TASK-20260718-011-ai-assistant-cost-governance                  | Integration Lead + Architecture/API reviewer      | scoped_verified_dormant      |
| 2026-07-19 | Activated and observed ChinaTech employee order text with explicit v2 minimal reasoning, one-attempt durable settlement and privacy-safe audit; all write/Vision/public paths remain off   | TASK-20260718-014-ai-assistant-live-pilot                       | Integration Lead                                  | scoped_verified_single_store |
