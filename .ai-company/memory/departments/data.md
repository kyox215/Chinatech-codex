...
- `TASK-20260718-012-workspace-integration-release` applied only `20260718150000_neutralize_store_settings_identity_defaults.sql` from a detached exact-migration worktree after a full dry-run exposed separately gated Inventory V2 migrations. The migration contains no row DML; 7-row count, max `updated_at`, aggregate fingerprint, RLS, policy and ACL fingerprints were unchanged. Inventory V2 `20260718175622` and `20260718181148` remain pending and must not be bundled with unrelated releases.
- `TASK-20260718-013-inventory-v2-production-canary` supersedes that pending statement for the V2 slice: AI cost governance plus the three V2 migrations are applied in linked order; ten new tables are RLS-on with zero policies/browser grants and nine checked functions are invoker with empty search path. Chinatech reconciliation is healthy, rollback-only intake/sale leaves zero residual rows, V1 inventory remains 5 and V1 mutations remain enabled. This is one-store evidence, not authorization for other stores or V1 retirement.
...
| DATA-20260718-005 | Inventory V2 is active only for Chinatech while V1 remains the rollback path | A second-store rollout or early V1 retirement could split inventory truth beyond the proven scope | Data + Security + Operations + Owner | keep one-store allowlist; require fresh reconcile/rollback/observation evidence before expansion | contained_by_single_store_canary |
| DATA-20260718-006 | Store lifecycle purge retry-baseline proof can accept a retry path without independently proving the original baseline | A future worker/scheduler activation could authorize unsafe permanent deletion | Data + Security + Operations + Owner | fix and adversarially verify retry-baseline semantics before any purge worker, scheduler or flag activation | blocked_activation |
...
| 2026-07-18 | Verified an unapplied AI quota migration on PostgreSQL 17 with atomic reserve/finalize/release, IANA periods, limit tightening and conservative settlement; production apply remains D4 | TASK-20260718-011-ai-assistant-cost-governance | Integration Lead + Data/Security reviewer | verified_file_local_only |
| 2026-07-18 | Applied one exact no-DML store-default migration, preserved row/RLS/ACL fingerprints and held both Inventory V2 migrations behind D4 | TASK-20260718-012-workspace-integration-release | Data/Security reviewers + Integration Lead | scoped_verified |
